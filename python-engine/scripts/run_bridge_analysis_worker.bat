...
if defined MOUTH_OF_TRUTH_PYTHON (
  ...
  "%MOUTH_OF_TRUTH_PYTHON%" -m mouth_of_truth.runners.bridge_analysis_worker
  exit /b %ERRORLEVEL%
)

if exist "%PYTHON_RUNTIME_ROOT_PATH%\python.exe" (
  set "PYTHONPATH=%PYTHON_MODULE_ROOT_PATH%"
  "%PYTHON_RUNTIME_ROOT_PATH%\python.exe" -m mouth_of_truth.runners.bridge_analysis_worker
  exit /b %ERRORLEVEL%
)

if exist "%PYTHON_RUNTIME_ROOT_PATH%\Scripts\python.exe" (
  set "PYTHONPATH=%PYTHON_MODULE_ROOT_PATH%"
  "%PYTHON_RUNTIME_ROOT_PATH%\Scripts\python.exe" -m mouth_of_truth.runners.bridge_analysis_worker
  exit /b %ERRORLEVEL%
)
...
if exist "%SystemRoot%\py.exe" (
  set "PYTHONPATH=%PYTHON_MODULE_ROOT_PATH%"
  "%SystemRoot%\py.exe" -3 -m mouth_of_truth.runners.bridge_analysis_worker
  exit /b %ERRORLEVEL%
)
...
for %%P in ("python.exe") do (
  if exist "%%~$PATH:P" (
    set "PYTHONPATH=%PYTHON_MODULE_ROOT_PATH%"
    "%%~$PATH:P" -m mouth_of_truth.runners.bridge_analysis_worker
    exit /b %ERRORLEVEL%
  )
)
...
set "PYTHONPATH=%PYTHON_MODULE_ROOT_PATH%"
"%CONDA_EXECUTABLE_PATH%" run --no-capture-output -n "%CONDA_ENVIRONMENT_NAME%" python -m mouth_of_truth.runners.bridge_analysis_worker
exit /b %ERRORLEVEL%
